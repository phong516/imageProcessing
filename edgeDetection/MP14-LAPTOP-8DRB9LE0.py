        ...
        for y in range(1, imgG.height - 1):
            ...
            for i in range (0, 3):
                for j in range (0, 3):
                    ...
                    gY += sobelY[i, j] * G   
            Mxy = abs(gX) + abs(gY)
            if (Mxy <= threshold): imgEdge.putpixel((x, y), (0, 0, 0))  #background - black
            else: imgEdge.putpixel((x, y), (255, 255, 255))             #edge - white
...
cv2.imshow('Anh Edge Detection GrayScale', anhEdgeDetection)
cv2.waitKey(0)
